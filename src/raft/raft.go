...
type Raft struct {
	...
	state ServerState // Leader, Follower, or Candidate

	currentTerm int   // latest term seen by server
	votedFor    int   // id of server that received vote in last term
	log         []Log // per server log
...
func (rf *Raft) randElectionTimeout() time.Duration {
	r := MinElectionTimeout + rand.Intn((MaxElectionTimeout - MinElectionTimeout))
	return time.Millisecond * time.Duration(r)
}

// returns current term & whether server is leader
